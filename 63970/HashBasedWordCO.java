...
public class HashBasedWordCO {
	...
	public void fillCoMat(String text, String[] keyWords) {
		...
		for (Iterator<String> iter = sentences.iterator(); iter.hasNext();) {
			String[] splitedOnes = iter.next().split("\\s+");
			int size = splitedOnes.length;
			for (int i = 0; i < size; i++) {
				if (coMat.get(splitedOnes[i]) != null) {
					//we choosed as the centered window, so that we can find the adjacent of the w.
					int w = windowSize;
					// a represents the left side
					int a = i - w;
					// b represents the right side
					int b = i + w;
					//As described on the above(for the Left side of the edge) the center of the window can not be on the out of the window if it is just equal it to 0
					if (a< 0) {
						a = 0;
					}
					//this loop starts from the beginning of the window till reaching out the right edge of the window
					for (int j = a; j <= b; j++) {
						//checking the conditions
						if (j != i && j < size) {
							//incrementing 
							coMat.get(splitedOnes[i]).increment(splitedOnes[j]);
						}
					}
				}
